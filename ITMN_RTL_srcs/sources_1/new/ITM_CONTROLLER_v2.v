...
// ============================================================================
// ITM_Top_v2 — D1 separation fork of ITM_Top (functional twin under no defines).
//
// Three build modes selected via Verilog `define at synth time:
//   (no define)        Full ITM controller (same behavior as ITM_Top)
//   MAMBA_ONLY         Strip P1+BR+FIN+CASCADE state arms; S_IDLE start jumps
//                      directly to S_NORM_M1A_SQ_READ. For Mamba_Top OOC synth.
//   INCEPTION_ONLY     Strip M*/NORM Mamba state arms; S_BR_NEXT skips Mamba
//                      and goes straight to S_FIN_READ. For Inception_Top OOC.
//
// Wrappers: Mamba_Top.v / Inception_Top.v instantiate this module under the
// corresponding define so Vivado dead-code-eliminates the unused datapath.
// Source-of-truth original (ITM_CONTROLLER.v / module ITM_Top) is unmodified.
// ============================================================================
        ...
        if (rst) begin
            ...
            S_IDLE: begin
                if (start) begin
                    ...
                    k_idx <= 0; substep <= 0; branch_id <= 0; s_idx <= 0; c_grp_br <= 0;
`ifdef MAMBA_ONLY
                    // D1 Mamba-only: skip P1+BR, start at RMSNorm M1A directly.
                    done_phase1 <= 1; done_inception <= 1;
                    bank_sel    <= 1;
                    norm_sq_acc <= 40'd0;
                    ...
                    if (c_grp_br != br_grp_last) begin
                        ...
                        case (branch_id)
                            ...
                            3'd4: begin
                                done_inception <= 1;
                                t_cnt <= 0; t_cnt_zero;
`ifdef INCEPTION_ONLY
                                // D1 Inception-only: skip Mamba phase, go straight to FIN.
                                // FIN reads A_MAMBA_OUT (never written here) → BRAM init 0
                                // → relu(0)=0 → final = relu(bn(inc)) which is the
                                // standalone Inception block output.
                                done_mamba <= 1;
                                c_grp <= 0; bank_sel <= 0;
...
`endif // !MAMBA_ONLY  (end P1+BR strip-eligible block)

`ifndef INCEPTION_ONLY
            // --------------------------------------------------------
            ...
            S_M8_NEXT: begin
                ...
                if (c_grp == ch_out_last[2:0]) begin  // ch_out_last groups
                    ...
                    if (t_cnt == t_last) begin
                        ...
                        bank_sel  <= 0; m_we <= 0;
`ifdef MAMBA_ONLY
                        // D1 Mamba-only: M8 done = block done. Skip FIN.
                        done_all <= 1;
                        state    <= S_DONE;
`else
                        c_rd_addr <= C_INC_SCALE;
...
`endif // !INCEPTION_ONLY  (end M*/NORM strip-eligible block)
...
`endif // !MAMBA_ONLY  (end FIN+CASCADE strip-eligible block)
