...
// LUT ROM init path — Vivado synth CWD is .runs/synth_1/, sim CWD is xsim/.
// Absolute path avoids CWD ambiguity between synth and sim.
`ifndef LUT_DIR
  `define LUT_DIR "C:/Users/ADMIN/Downloads/DoAn1/DoAn1/ITMN_FASTMAMBA/golden_all"
`endif
...
// ============================================================================
// FastMamba workload port — Mamba2-130M config
//   d_model=768 (48 grp), d_inner=1536 (96 grp), d_state=128 (8 s-grp),
//   nheads=24, n_layer=24. No dt_rank (Mamba2 uses per-head scalar dt).
// ============================================================================

// State-dim geometry (each s-group = 16 lanes of h)
`define N_STATE           8'd128            // Mamba2 d_state
`define N_STATE_GRP       4'd8              // d_state / 16
`define N_STATE_GRP_MAX   4'd7              // N_STATE_GRP - 1
`define N_STATE_GRP_W     3                 // bits for ctr_s

// Widened dim ports for LightMamba/MARCA family (up to Mamba2-2.7B).
// 9-bit: 511 grp max x 16 lanes = 8176 d_model/d_inner.
// Mamba2-2.7B: d_inner=5120 -> 320 grp fits.
`define CH_OUT_W          9                 // supports d_model up to 8176
`define CH_M_W            9                 // supports d_inner up to 8176
`define CTR_G_W           9                 // ctr_g width

// LightMamba family workload configs (per-model TB constants)
//   Mamba2-130M : d_model=768,  d_inner=1536, n_layer=24, nheads=24, headdim=64
//   Mamba2-370M : d_model=1024, d_inner=2048, n_layer=48, nheads=32, headdim=64
//   Mamba2-780M : d_model=1536, d_inner=3072, n_layer=48, nheads=48, headdim=64
//   Mamba2-1.3B : d_model=2048, d_inner=4096, n_layer=48, nheads=64, headdim=64
//   Mamba2-2.7B : d_model=2560, d_inner=5120, n_layer=64, nheads=80, headdim=64

// Mamba2-130M (default reference)
`define M2_D_MODEL        16'd768
`define M2_D_INNER        16'd1536
`define M2_D_STATE        16'd128
`define M2_NHEADS         8'd24
`define M2_HEADDIM        8'd64
`define M2_N_LAYER        8'd24
`define M2_CH_OUT_GRP     9'd48             // d_model/16
`define M2_CH_M_GRP       9'd96             // d_inner/16

// DMA reload cycle model (paper-fair): 256b/cyc bandwidth over AXI
`define DMA_WORD_CYC      1                 // 1 cyc per 256b word streamed
