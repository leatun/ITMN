...
// ============================================================================
// tb_Mamba_Top_VIM_TI - cycle benchmark for Vision Mamba Tiny (Vim-Ti).
//
...
//
// Vim-Ti config (state-spaces/mamba defaults for Vim Tiny):
...
module tb_Mamba_Top_VIM_TI;

    localparam CH_OUT_V        = 9'd12;
    localparam CH_M_V          = 9'd24;
    localparam DT_RANK_V       = 8'd12;
    localparam N_STATE_GRP_V   = 4'd1;
    localparam USE_M5_V        = 1'b1;
    localparam XP_OUT_GRP_V    = 5'd3;    // (12+32)/16 = 3
    localparam integer N_LAYER = 24;
    localparam integer SEQ_LEN = 197;     // Full Vim-Ti @ 224x224
    localparam T_TEST          = 3;

    // ---- DMA per layer (Mamba1 Vim-Ti): ~15704 words ----
    ...
    localparam integer DMA_CYC_PER_LAYER = 16184;
    ...
    initial begin
        ...
        @(negedge clk); start = 0;
        cyc_start = $time / 10;
        $display("[CFG] Vim-Ti (D=192 CH_OUT=%0d CH_M=%0d dt=%0d N_STATE_GRP=%0d)",
        ...
        $display("");
        $display("========================================================");
        $display("     tb_Mamba_Top_VIM_TI - cycle report (T=%0d, 1 layer)", T_TEST);
        $display("========================================================");
        $display("Config: Vision Mamba Tiny (Vim-Ti), 1 layer, %0d token(s)", T_TEST);
        $display("  D=192 (12 grp) d_inner=384 (24 grp) d_state=16 (1 s-grp)");
        $display("  dt_rank=12 XP=3 L=24 patch=16 seq_len=197 (image 224x224)");
        $display("");
        ...
        $display("  Mamba-X Vim-Ti     : normalized 11.6x GPU speedup, target 30W TDP");
