...
// ============================================================================
// tb_Mamba_Top_VIM_B - cycle benchmark for Vision Mamba Tiny (Vim-B).
//
...
//
// Vim-B config (state-spaces/mamba defaults for Vim Tiny):
...
module tb_Mamba_Top_VIM_B;

    localparam CH_OUT_V        = 9'd48;
    localparam CH_M_V          = 9'd96;
    localparam DT_RANK_V       = 8'd48;
    localparam N_STATE_GRP_V   = 4'd1;
    localparam USE_M5_V        = 1'b1;
    localparam XP_OUT_GRP_V    = 5'd5;    // (12+32)/16 = 3
    localparam integer N_LAYER = 24;
    localparam integer SEQ_LEN = 197;     // Full Vim-B @ 224x224
    localparam T_TEST          = 3;

    // ---- DMA per layer (Mamba1 Vim-B): ~15704 words ----
    ...
    localparam integer DMA_CYC_PER_LAYER = 233768;
    ...
    initial begin
        ...
        @(negedge clk); start = 0;
        cyc_start = $time / 10;
        $display("[CFG] Vim-B (D=192 CH_OUT=%0d CH_M=%0d dt=%0d N_STATE_GRP=%0d)",
        ...
        $display("");
        $display("========================================================");
        $display("     tb_Mamba_Top_VIM_B - cycle report (T=%0d, 1 layer)", T_TEST);
        $display("========================================================");
        $display("Config: Vision Mamba Tiny (Vim-B), 1 layer, %0d token(s)", T_TEST);
        $display("  D=768 (48 grp) d_inner=1536 (96 grp) d_state=16 (1 s-grp)");
        $display("  dt_rank=48 XP=5 L=24 patch=16 seq_len=197 (image 224x224)");
        $display("");
        ...
        $display("  Mamba-X Vim-B     : normalized 11.6x GPU speedup, target 30W TDP");
