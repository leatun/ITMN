...
module tb_Mamba_Top_MAMBA2_2_7B;

    // ---- Mamba2-130M config (default) ----
    //   d_model=768 -> CH_OUT=48   d_inner=1536 -> CH_M=96
    //   n_layer=24  nheads=24 headdim=64  d_state=128 -> N_STATE_GRP=8
    localparam CH_OUT_V        = 9'd160;
    localparam CH_M_V          = 9'd320;
    localparam DT_RANK_V       = 4'd0;
    localparam N_STATE_GRP_V   = 4'd8;
    localparam USE_M5_V        = 1'b0;
    localparam XP_OUT_GRP_V    = 5'd21;   // dt(24)+B(128)+C(128)/16 = 18
    localparam integer N_LAYER = 64;
    localparam T_TEST          = 3;

    // ---- DMA reload cost (paper-fair MODEL, per-layer) ----
    // Serial DMA streaming @ 256b/cyc. Real HW overlaps via double-buffer.
    //   W_InProj (dt+B+C+x+z): (2*d_inner+2*ngroups*d_state+nheads)*d_model/16
    //   W_OutProj:             d_inner*d_model/16
    //   W_DW:                  d_inner*4/16
    //   W_A (per-head):        nheads*d_state/16
    //   Norms + biases:        ~200
    // 130M: (3352*768 + 1536*768 + 1536*4 + 24*128)/16 + 200 ~= 235,400
    localparam integer DMA_CYC_PER_LAYER = 2513480;
