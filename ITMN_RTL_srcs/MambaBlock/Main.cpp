...
// Các hàm template transpose giữ nguyên, không cần thay đổi
...
void ITMN_mamba_block(
    ...
    for (int i = 0; i < SEQ_LEN; ++i) {
        ...
        //linear(const model_dtype x[], model_dtype y[], const model_dtype* W, const model_dtype b[], int in_dim, int out_dim)

        //x_norm[SEQ_LEN][D_MODEL]
        //x_primary_expanded[SEQ_LEN][D_INNER]
        //in_proj1_weight| shape=(128, 64)
    }
    transpose_2d_SEQ_LEN_to_D_INNER(x_primary_expanded, x_primary_transposed);
    //x_primary_transposed[D_INNER][SEQ_LEN]
    ...
    for (int i = 0; i < D_INNER; ++i) for (int j = 0; j < D_STATE; ++j) A_cpp[i][j] = -std::exp(weights->A_log[i * D_STATE + j]);
    for (int d=0; d<D_INNER; ++d) for (int l=0; l<SEQ_LEN; ++l) for (int n=0; n<D_STATE; ++n) {
        discrete_A[d][l][n] = std::exp(A_cpp[d][n] * delta[d][l]);
        deltaB_u[d][l][n] = (delta[d][l] * B_raw[l][n]) * x_prime[d][l];
    }
    ...
    for (int d = 0; d < D_INNER; ++d) {
        for (int l = 0; l < SEQ_LEN; ++l) {
            scan_output_with_D[d][l] = scan_output_raw[d][l] + (weights->D[d] * x_prime[d][l]);
        }
    }
