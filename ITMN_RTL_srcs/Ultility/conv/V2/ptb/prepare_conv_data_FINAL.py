...
def run():
    ...
    save_folded("conv_x_input_real.txt", X_fixed)
    
    # Dùng kết quả Sim của Python làm Golden cho Verilog (để khớp 100%)
    save_folded("conv_y_golden_real.txt", Y_sim_fixed)
    
    save_folded("conv_y_golden_ptb.txt", Y_gold_fixed)
    
    # XUẤT WEIGHT ĐẢO NGƯỢC
    with open("conv_w_input_real.txt", "w") as f:
        for c in range(D_INNER):
            for k in range(KERNEL_SIZE): 
    ...
    with open("conv_b_input_real.txt", "w") as f:
