...
module tb_Mamba_Top_EMAMBA_FMNIST;

    // ---- Mamba1-130M config (MARCA workload) ----
    localparam CH_OUT_V        = 9'd2;
    localparam CH_M_V          = 9'd3;
    localparam DT_RANK_V       = 8'd2;    // Mamba1: dt_rank = ceil(d_model/16)
    localparam N_STATE_GRP_V   = 4'd1;     // Mamba1: d_state=16 -> 1 grp
    localparam USE_M5_V        = 1'b1;     // Mamba1: enable M5 dt-proj stage
    localparam XP_OUT_GRP_V    = 5'd3;     // (48+32)/16 = 5
    ...
    localparam integer DMA_CYC_PER_LAYER = 581;
