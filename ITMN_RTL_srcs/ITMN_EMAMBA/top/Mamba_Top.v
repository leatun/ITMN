...
module Mamba_Top (
    ...
    input  wire [8:0]   CH_OUT,      // Widened for Mamba2 (d_model up to 2048 → 128 grp)
    input  wire [8:0]   CH_M,        // Widened for Mamba2 (d_inner up to 2048 → 128 grp)
    input  wire [7:0]   DT_RANK,
    ...
    wire [9:0]  ch_m_act = (CH_M == 9'd0) ? 8'd128 : CH_M;
    wire [13:0] d_inner  = {ch_m_act, 4'b0};   // Widened: up to 2048
    // H init limit = d_inner * N_STATE_GRP (Mamba1=1, Mamba2=8)
    wire [17:0] h_init_limit = (N_STATE_GRP == 4'd8) ? {1'b0, d_inner, 3'b0} :
                                                       {4'b0, d_inner};
    wire [12:0] d_model  = {CH_OUT, 4'b0};     // Widened: up to 2032
    wire [9:0]  t_last   = T_MAX - 10'd1;
    // Generalized log2 for d_model (ITMN B0-B4 + Mamba2 d_model=768 → log2=9)
    wire [3:0]  log2_dm  =
        (CH_OUT == 9'd4)   ? 4'd2  :   // d_model=64  (B0/B4 style)
        (CH_OUT == 9'd8)   ? 4'd3  :   // d_model=128
        (CH_OUT == 9'd16)  ? 4'd4  :   // d_model=256
        (CH_OUT == 9'd32)  ? 4'd5  :   // d_model=512
        (CH_OUT == 9'd48)  ? 4'd9  :   // d_model=768 (Mamba2-130M) — log2(768)=9.58→9
                             4'd10;    // d_model=1024+
    ...
    reg [3:0]   cur_stage;
    reg [9:0]   t_cnt;
    reg [8:0]   ctr_g;              // Widened 5→7-bit for Mamba2 (up to 96 grp)
    reg [10:0]  ctr_k;               // Widened 9→11-bit (up to d_inner=1536)
    reg [1:0]   tap_cnt;
    reg [7:0]   ctr_r;
    reg [8:0]   m3_wr_ptr;              // Widened 5→7-bit for Mamba2 CH_M=96
    ...
    wire [14:0]  c2_m6_delta_addr   = PT_DELTA + {6'b0, c2_g};
    wire [14:0]  c2_m6_u_addr       = PT_U     + {6'b0, c2_g};
    wire [14:0]  c2_m6_D_addr_const = C_D_PARAM_BASE + {6'b0, c2_g};
    wire [14:0]  c2_m7_z_addr       = PT_Z_GATE  + {6'b0, c2_g};
    wire [14:0]  c2_m7_wr_addr      = PT_Y_GATED + {6'b0, c2_g};
    ...
    wire [14:0] m1a_wr_addr = PT_X_INNER_CIRC
                              + ({13'b0, t_cnt[1:0]} * {5'b0, ch_m_act})
                              + {6'b0, ctr_g};
    wire [14:0] m1b_wr_addr = PT_Z_GATE + {6'b0, ctr_g};
    wire [14:0] m4_wr_addr  = PT_X_PROJ + {6'b0, ctr_g};
    ...
    wire [9:0]  mac_grp_last = mac_grp_count - 10'd1;
    ...
    // g_bank_idx: for PP, use ctr_g[8:1] = ctr_g/2 (both banks addressed identically).
    // For non-PP (M4/M8 in Phase 1), use full ctr_g.
    wire [8:0] g_bank_idx = pp_enable ? {1'b0, ctr_g[8:1]} : ctr_g;
    ...
    wire [8:0]  ctr_g_p1 = ctr_g + 9'd1;
    wire [8:0]  ctr_g_p2 = ctr_g + 9'd2;
    // Next group increment: +2 for PP (even→next even), +1 otherwise.
    wire [8:0]  ctr_g_step = pp_enable ? ctr_g_p2 : ctr_g_p1;
    // Next iter's g_bank_idx: for PP (ctr_g even, +2), next bank idx = ctr_g/2 + 1 = ctr_g_p2[8:1].
    wire [8:0]  g_bank_idx_next = pp_enable ? {1'b0, ctr_g_p2[8:1]} : ctr_g_p1;
    ...
    wire mac_last_iter = pp_enable ? (ctr_g_p1 == {1'b0, mac_grp_last[8:0]})
                                    : (ctr_g    == {1'b0, mac_grp_last[8:0]});

    wire [9:0]  inner_grp_last = ch_m_act - 10'd1;

    wire [9:0]  rn_x_per_t     = CH_OUT;
    wire [9:0]  rn_x_grp_last  = rn_x_per_t - 10'd1;
    wire [14:0] rn_x_per_t_ext = {5'b0, rn_x_per_t};
    wire [14:0] rn_t_offset_x  = t_cnt * rn_x_per_t_ext;
    wire [14:0] rn_ap_rd_addr_g0  = PT_INPUT + rn_t_offset_x + {6'b0, ctr_g};
    wire [14:0] rn_ap_rd_addr_g1   = PT_INPUT + rn_t_offset_x + {6'b0, ctr_g_p1};
    wire [14:0] rn_ap_c_addr_g0  = C_W_NORM_BASE + {6'b0, ctr_g};
    wire [14:0] rn_ap_c_addr_g1   = C_W_NORM_BASE + {6'b0, ctr_g_p1};
    wire [14:0] rn_wr_addr     = PT_X_NORM + {6'b0, ctr_g};
    ...
    wire [10:0] rn_sq_last     = {1'b0, rn_x_grp_last};
    ...
    wire [14:0] m2_rd_addr_tap0 = PT_X_INNER_CIRC
                                    + ({13'b0, eff_t_slot} * {5'b0, ch_m_act})
                                    + {6'b0, ctr_g};
    ...
    wire [14:0] m2_rd_addr_tap1 = PT_X_INNER_CIRC
                               + ({13'b0, eff_t_slot_p1} * {5'b0, ch_m_act})
                               + {6'b0, ctr_g};
    wire [14:0] m2_rd_addr_tap2 = PT_X_INNER_CIRC
                               + ({13'b0, eff_t_slot_p2} * {5'b0, ch_m_act})
                               + {6'b0, ctr_g};

    wire [14:0] m2_w_addr_tap0 = W_DW_BASE + ({5'b0, ctr_g} * `M2_WEIGHT_STRIDE) + {13'b0, tap_cnt};
    wire [14:0] m2_w_addr_tap1      = W_DW_BASE + ({5'b0, ctr_g} * `M2_WEIGHT_STRIDE) + {12'b0, tap_p1};
    wire [14:0] m2_w_addr_tap2      = W_DW_BASE + ({5'b0, ctr_g} * `M2_WEIGHT_STRIDE) + {12'b0, tap_p2};
    wire [14:0] m2_c_addr      = C_B_DW_BASE + {6'b0, ctr_g};
    wire [14:0] m2_wr_addr        = PT_X_CONV + {6'b0, ctr_g};
    ...
    wire [14:0] m2_rd_addr_gnext = PT_X_INNER_CIRC
                                     + ({13'b0, eff_t_slot_tap0} * {5'b0, ch_m_act})
                                     + {6'b0, ctr_g_p1};
    wire [14:0] m2_w_addr_gnext = W_DW_BASE + ({5'b0, ctr_g_p1} * `M2_WEIGHT_STRIDE);
    wire [14:0] m2_c_addr_gnext  = C_B_DW_BASE + {6'b0, ctr_g_p1};
    ...
    wire [7:0]   dt_rank_last    = DT_RANK - 8'd1;
    wire [7:0]   ctr_r_p1         = ctr_r + 8'd1;
    wire [7:0]   ctr_r_p2         = ctr_r + 8'd2;
    wire [14:0]  m5_w_addr_r0   = W_DTPROJ_BASE + ({6'b0, ctr_g} * {7'b0, DT_RANK}) + {7'b0, ctr_r};
    wire [14:0]  m5_w_addr_r1    = W_DTPROJ_BASE + ({6'b0, ctr_g} * {7'b0, DT_RANK}) + {7'b0, ctr_r_p1};
    wire [14:0]  m5_w_addr_r2    = W_DTPROJ_BASE + ({6'b0, ctr_g} * {7'b0, DT_RANK}) + {7'b0, ctr_r_p2};
    wire [14:0]  m5_c_addr    = C_B_DT_BASE + {6'b0, ctr_g};
    wire [14:0]  m5_wr_addr      = PT_DELTA + {6'b0, ctr_g};
    // Next-group prefetch helpers (for WRITE→MAC fusion, skip W_PREF/W_WAIT)
    wire [14:0]  m5_w_addr_gnext_r0 = W_DTPROJ_BASE + ({6'b0, ctr_g_p1} * {7'b0, DT_RANK});
    wire [14:0]  m5_w_addr_gnext_r1 = m5_w_addr_gnext_r0 + 15'd1;
    wire [14:0]  m5_c_addr_gnext = C_B_DT_BASE + {6'b0, ctr_g_p1};

    wire [14:0] m7_z_addr    = PT_Z_GATE + {6'b0, ctr_g};
    wire [14:0] m7_z_addr_p1 = PT_Z_GATE + {6'b0, ctr_g} + 15'd1;
    wire [14:0] m7_y_addr    = PT_Y_SSM  + {6'b0, ctr_g};
    wire [14:0] m7_wr_addr   = PT_Y_GATED + {6'b0, ctr_g};
    ...
    wire [14:0]  m6_delta_addr   = PT_DELTA + {6'b0, ctr_g};
    wire [14:0]  m6_u_addr       = PT_U     + {6'b0, ctr_g};
    wire [14:0]  m6_D_addr_const = C_D_PARAM_BASE + {6'b0, ctr_g};
    wire [14:0]  m6_y_ssm_wr_addr = PT_Y_SSM + {6'b0, ctr_g};
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            case (state)
                ...
                S_M3_1: begin
                    if (ctr_g <= inner_grp_last) begin
                        m_rd_addr <= PT_X_CONV + {6'b0, ctr_g};
                    end
                    ctr_g <= ctr_g + 9'd1;
                    if (ctr_g >= 9'd2) begin
                        m_we      <= 1'b1;
                ...
                S_M7_7: begin
                    ...
                    if (ctr_g_p1 == inner_grp_last[8:0]) begin
        ...
        if (rst) begin
            ...
            case (c2_state)
                ...
                S_C2_WRITE2: begin
                    ...
                    if (c2_g == inner_grp_last[8:0]) begin
                        c2_done  <= 1'b1;
                        c2_state <= S_C2_DONE;
                    end else begin
                        c2_g     <= c2_g + 9'd2;                // next odd
                        c2_l     <= 4'd0;
                        c2_s     <= 3'd0;
                        c2_load  <= 3'd0;
                        c2_state <= S_C2_M6_LOAD1;
