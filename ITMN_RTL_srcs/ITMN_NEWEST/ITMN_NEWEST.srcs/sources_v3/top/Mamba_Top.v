...
module Mamba_Top (
    ...
    reg [3:0]   cur_stage;
    reg [9:0]   t_cnt;
    reg [4:0]   ctr_g;              // output-group counter (cross-stage: RN, M1A/B, M2..M8)
    reg [8:0]   ctr_k;
    reg [1:0]   tap_cnt;
    reg [4:0]   ctr_r;
    reg [4:0]   m3_wr_ptr;
    ...
    wire [14:0] m1a_wr_addr = PT_X_INNER_CIRC
                              ...
                              + {10'b0, ctr_g};
    wire [14:0] m1b_wr_addr = PT_Z_GATE + {10'b0, ctr_g};
    wire [14:0] m4_wr_addr  = PT_X_PROJ + {10'b0, ctr_g};
    ...
    wire [14:0] w_grp_base   = mac_w_base + ({10'b0, ctr_g} * mac_len_ext);

    wire [8:0]  ctr_k_p1 = ctr_k + 9'd1;
    wire [8:0]  ctr_k_p2 = ctr_k + 9'd2;
    wire [8:0]  ctr_k_p4 = ctr_k + 9'd4;

    wire [14:0] mac_rd_addr_k0 = mac_in_base + {11'b0, ctr_k[8:4]};
    wire [14:0] mac_rd_addr_k2  = mac_in_base + {11'b0, ctr_k_p2[8:4]};
    wire [14:0] mac_rd_addr_k4  = mac_in_base + {11'b0, ctr_k_p4[8:4]};
    wire [14:0] mac_w_addr_k0 = w_grp_base + {6'b0, ctr_k};
    wire [14:0] mac_w_addr_k2  = w_grp_base + {6'b0, ctr_k_p2};
    ...
    wire [14:0] rn_ap_rd_addr_g0  = PT_INPUT + rn_t_offset_x + {10'b0, ctr_g};
    wire [14:0] rn_ap_rd_addr_g1   = PT_INPUT + rn_t_offset_x + {10'b0, ctr_g_p1};
    wire [14:0] rn_ap_c_addr_g0  = C_W_NORM_BASE + {10'b0, ctr_g};
    wire [14:0] rn_ap_c_addr_g1   = C_W_NORM_BASE + {10'b0, ctr_g_p1};
    wire [14:0] rn_wr_addr     = PT_X_NORM + {10'b0, ctr_g};
    wire [14:0] rn_sq_rd_addr_k0  = PT_INPUT + rn_t_offset_x + {11'b0, ctr_k[3:0]};
    wire [14:0] rn_sq_rd_addr_k1   = PT_INPUT + rn_t_offset_x + {11'b0, ctr_k_p1[3:0]};
    wire [14:0] rn_sq_rd_addr_k2   = PT_INPUT + rn_t_offset_x + {11'b0, ctr_k_p2[3:0]};
    ...
    wire [14:0] m2_w_addr_tap0 = W_DW_BASE + ({9'b0, ctr_g} * `M2_WEIGHT_STRIDE) + {13'b0, tap_cnt};
    wire [14:0] m2_w_addr_tap1      = W_DW_BASE + ({9'b0, ctr_g} * `M2_WEIGHT_STRIDE) + {12'b0, tap_p1};
    wire [14:0] m2_w_addr_tap2      = W_DW_BASE + ({9'b0, ctr_g} * `M2_WEIGHT_STRIDE) + {12'b0, tap_p2};
    wire [14:0] m2_c_addr      = C_B_DW_BASE + {10'b0, ctr_g};
    wire [14:0] m2_wr_addr        = PT_X_CONV + {10'b0, ctr_g};
    ...
    wire [14:0] m2_rd_addr_gnext = PT_X_INNER_CIRC
                                     + ({13'b0, eff_t_slot_tap0} * {10'b0, ch_m_act})
                                     + {10'b0, ctr_g_p1};
    wire [14:0] m2_w_addr_gnext = W_DW_BASE + ({9'b0, ctr_g_p1} * `M2_WEIGHT_STRIDE);
    wire [14:0] m2_c_addr_gnext  = C_B_DW_BASE + {10'b0, ctr_g_p1};
    ...
    wire [14:0]  m5_w_addr_r0   = W_DTPROJ_BASE + ({10'b0, ctr_g} * {11'b0, DT_RANK}) + {10'b0, ctr_r};
    wire [14:0]  m5_w_addr_r1    = W_DTPROJ_BASE + ({10'b0, ctr_g} * {11'b0, DT_RANK}) + {10'b0, ctr_r_p1};
    wire [14:0]  m5_w_addr_r2    = W_DTPROJ_BASE + ({10'b0, ctr_g} * {11'b0, DT_RANK}) + {10'b0, ctr_r_p2};
    wire [14:0]  m5_c_addr    = C_B_DT_BASE + {10'b0, ctr_g};
    wire [14:0]  m5_wr_addr      = PT_DELTA + {10'b0, ctr_g};
    // Next-group prefetch helpers (for WRITE→MAC fusion, skip W_PREF/W_WAIT)
    wire [14:0]  m5_w_addr_gnext_r0 = W_DTPROJ_BASE + ({10'b0, ctr_g_p1} * {11'b0, DT_RANK});
    wire [14:0]  m5_w_addr_gnext_r1 = m5_w_addr_gnext_r0 + 15'd1;
    wire [14:0]  m5_c_addr_gnext = C_B_DT_BASE + {10'b0, ctr_g_p1};

    wire [14:0] m7_z_addr    = PT_Z_GATE + {10'b0, ctr_g};
    wire [14:0] m7_z_addr_p1 = PT_Z_GATE + {10'b0, ctr_g} + 15'd1;
    wire [14:0] m7_y_addr    = PT_Y_SSM  + {10'b0, ctr_g};
    wire [14:0] m7_wr_addr   = PT_Y_GATED + {10'b0, ctr_g};
    ...
    wire [8:0]   m6_c = {ctr_g[3:0], ctr_l};

    wire [14:0]  m6_w0_addr      = PT_X_PROJ + 15'd0;
    wire [14:0]  m6_B_addr       = PT_X_PROJ + 15'd1;
    wire [14:0]  m6_C_addr       = PT_X_PROJ + 15'd2;
    wire [14:0]  m6_delta_addr   = PT_DELTA + {10'b0, ctr_g};
    wire [14:0]  m6_u_addr       = PT_U     + {10'b0, ctr_g};
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            case (state)
                ...
                S_M3_1: begin
                    if (ctr_g <= inner_grp_last) begin
                        m_rd_addr <= PT_X_CONV + {10'b0, ctr_g};
                    end
                    ctr_g <= ctr_g + 5'd1;
                    if (ctr_g >= 5'd2) begin
                        m_we      <= 1'b1;
                        m_wr_addr <= PT_U + {10'b0, m3_wr_ptr};
                        m_wr_data <= silu_out_w;
                        m3_wr_ptr <= m3_wr_ptr + 5'd1;
                ...
                // -----------------------------------------------------------
                // M6 load template (replaces 9 states, 9→7 cyc first entry,
                // 4 cyc re-entry). Slot map:
                //   0=w0(m6_w0_addr) 1=B(m6_B_addr)   2=C(m6_C_addr)
                //   3=delta          4=u
                //   D from const bus latches with delta at slot 3.
                //   A weight + H addr side-issued at slot-1 latch (first entry)
                //   or at ISSUE (re-entry). 2-cyc BRAM latency alignment.
                // -----------------------------------------------------------
                S_M6_1: begin
