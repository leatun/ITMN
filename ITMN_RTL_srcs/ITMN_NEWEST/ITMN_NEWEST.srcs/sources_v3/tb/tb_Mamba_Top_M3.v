...
// tb_Mamba_Top_M3 — byte-exact M3 (SiLU LUT pass).
//
// Input  X_CONV in ram_a @ 4000  — Mam_X_Conv_FP (d_inner, T)
// Output U      in ram_b @ 0     — compare Mam_U_Silu_FP (d_inner, T)
//
// Packing identical: word @ (t * d_inner/16 + c_grp) packs 16 channels at t.

module tb_Mamba_Top_M3;
    ...
    initial begin
        $readmemh("golden_all/block_00_layer00/Mam_X_Conv_FP.txt", x_mem);
        $readmemh("golden_all/block_00_layer00/Mam_U_Silu_FP.txt", exp_mem);

        rst=1; @(posedge clk); @(posedge clk); @(negedge clk); rst=0; @(posedge clk);

        // Load X_CONV → ram_a @ A_X_CONV_BASE
        for (t_cur=0; t_cur<T_TEST; t_cur=t_cur+1) begin
            for (c_grp=0; c_grp<(D_INNER/16); c_grp=c_grp+1) begin
                ...
                dma_wr(2'd0, A_X_CONV_BASE + t_cur*(D_INNER/16) + c_grp, word_tmp);
            end
        end
        @(negedge clk); dma_write_en=0;

        @(negedge clk); start=1; @(negedge clk); start=0;
        $display("[FSM] M3 running...");
        wait (done_stage==1'b1);
        @(negedge clk); wait (done_stage==1'b0);

        // Readback ram_b @ B_U_BASE
        @(negedge clk); dma_read_en=1; dma_rtarget=2'd1;
        for (t_cur=0; t_cur<T_TEST; t_cur=t_cur+1) begin
            for (c_grp=0; c_grp<(D_INNER/16); c_grp=c_grp+1) begin
                @(negedge clk); dma_raddr = B_U_BASE + t_cur*(D_INNER/16) + c_grp;
        ...
        $display("");
        $display("---- tb_Mamba_Top_M3 summary: compares=%0d errors=%0d ----", compares, errors);
        if (errors==0) $display("===== TB M3 BYTE-EXACT PASS =====");
        else           $display("===== TB M3 FAIL =====");
        $finish;
    end
